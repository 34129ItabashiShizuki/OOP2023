...
namespace SampleEntityFramewaork {
    class Program {
        ...
        private static void Exercise1_4() {
            using (var db = new BooksDbContext()) {
                var matu = db.Books.OrderBy(s => s.PublishedYear).ToList();
                var sima = matu.Take(3);
                foreach (var book in sima) {
        ...
        private static void Exercise1_5() {
            using (var db = new BooksDbContext()) {
                var ni = db.Books.OrderByDescending(s => s.Author.Birthday).ToList();
                foreach (var book in ni) {
                    Console.WriteLine($"{book.Author.Name} : {book.Title} {book.PublishedYear} {book.Author.Birthday.Year}");
                }
            }
        }
        ...
        static void DisplayAllBooks() {
            ...
            //Console.ReadLine(); //コンソールの画面をすぐにけさないためにキー入力待ちにする
        }
